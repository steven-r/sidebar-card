...
// ------------------------------------------------------------------------------------------
const SIDEBAR_CARD_VERSION = '0.1.10.0.0';
...
class SidebarCard extends s$1 {
    ...
    getWidth(type) {
        const w = {
            mobile: this.config.width ?
                typeof (this.config.width) == typeof (Number) ? this.config.width : this.config.width.mobile ?? 25
                : 25,
            tablet: this.config.width ?
                typeof (this.config.width) == typeof (Number) ? this.config.width : this.config.width.tablet ?? 25
                : 25,
            desktop: this.config.width ?
                typeof (this.config.width) == typeof (Number) ? this.config.width : this.config.width.desktop ?? 25
                : 25,
        };
        switch (type) {
    ...
    render() {
      ...
      <div class="sidebar-inner">
        ...
        ${this.cards && this.cards.length > 0
            ? x `<div class="sidebarcards">
            ${this.cards.map(c => x `<div>${c}</div>`)}
          </div>`
            : x ``}
    ...
    async _finishSetup() {
        ...
        if (this.config.cards) {
            this.cards = await Promise.all(this.config.cards.map(async (card) => {
...
__decorate([
    t()
], SidebarCard.prototype, "config", void 0);
__decorate([
    t()
], SidebarCard.prototype, "current_date", void 0);
__decorate([
    t()
], SidebarCard.prototype, "current_time", void 0);
__decorate([
    n$2()
], SidebarCard.prototype, "templateLines", void 0);
__decorate([
    n$2()
], SidebarCard.prototype, "cards", void 0);
// hides (if requested) the HA header, HA footer and/or HA sidebar and hides this sidebar if configured so
function updateStyling(appLayout, card) {
...
function subscribeEvents(appLayout, card, contentContainer, sidebar) {
...
async function buildSidebar() {
    ...
    if (lovelace && lovelace.config.sidebar) {
        const sidebarConfig = Object.assign({}, DEFAULT_CONFIG, lovelace.config.sidebar);
        const root = getRoot();
